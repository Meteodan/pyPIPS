...
def getradarfilelist(radar_dir, radar_save_dir, starttime=None, stoptime=None, platform='NEXRAD', radar_name=None,
                     el_req=0.5):
    ...
    if(platform == 'UMXP' or platform == 'SMARTR'):
        ...
        radar_filelist_filename = radar_name + '_' + starttime.strftime(tm.timefmt3).strip() + '_' +   \
            stoptime.strftime(tm.timefmt3).strip() + '_el' + str(el_req) + '_filelist.txt'
...
def readCFRadial_pyART(nexrad, el, radlat, radlon, radalt, file, sweeptime, fieldnames,
    ...
    sweepindex = (N.abs(elev_list - el)).argmin()
            ...
            if(not f and compute_kdp):
                tempfield = next(
                    (field_data['data'] for field_key,
                     field_data in radarsweep.fields.items() if field_key in ['PHI']),
                    N.empty(
                        (0)))
                if(tempfield.size):
                    ...
                    # field.
                    ncp_exists = any(
                        x in radarsweep.fields.keys() for x in [
...
#     azimuth = azimuth[:stopindex+1]
    ...
    azimuth_start_rad = azimuth_start_rad % (2. * N.pi)
...
def readsweeps2PIPS(fieldnames, pc, ib):
    ...
    if(not pc.loadradopt):
        ...
        sweepdict['azimuth_start_tarr'] = N.array(azimuth_start_tlist)
        sweepdict['azimuth_tarr'] = N.array(azimuth_tlist)
        sweepdict['rlat_tarr'] = N.array(rlat_tlist)
        sweepdict['rlon_tarr'] = N.array(rlon_tlist)
        sweepdict['ralt_tarr'] = N.array(ralt_tlist)
        ...
        if(pc.saveradopt):
            ...
            N.savez_compressed(radnpz_path, **sweepdict)
    ...
    if(pc.loadradopt):
        ...
        radnpz_file = N.load(radnpz_path)

...
class WRFDataHandler(DataHandler):
    ...
    def loadGrid(self):
        ...
        if(map_proj == 0): # No map projection (probably idealized run)
            ...
            xe, dummy = N.meshgrid(N.arange(nxe) * dx - dx/2., N.arange(nym) * dy) # Note, grid edges are offset by dx/2.
...
#           xc_rel = N.lib.stride_tricks.as_strided(xc_rel, strides=((0, ) + xc_rel.strides), shape=((nzm, ) + xc_rel.shape))
#           yc_rel = N.lib.stride_tricks.as_strided(yc_rel, strides=((0, ) + yc_rel.strides), shape=((nzm, ) + yc_rel.shape))
#           xe_rel = N.lib.stride_tricks.as_strided(xe_rel, strides=((0, ) + xe_rel.strides), shape=((nzm, ) + xe_rel.shape))
#           ye_rel = N.lib.stride_tricks.as_strided(ye_rel, strides=((0, ) + ye_rel.strides), shape=((nzm, ) + ye_rel.shape))

        xc = N.lib.stride_tricks.as_strided(xc, strides=((0, ) + xc.strides), shape=((nzm, ) + xc.shape))
        yc = N.lib.stride_tricks.as_strided(yc, strides=((0, ) + yc.strides), shape=((nzm, ) + yc.shape))
        xe = N.lib.stride_tricks.as_strided(xe, strides=((0, ) + xe.strides), shape=((nzm, ) + xe.shape))
        ye = N.lib.stride_tricks.as_strided(ye, strides=((0, ) + ye.strides), shape=((nzm, ) + ye.shape))
    ...
    def _readwrfmicroLIN(self):
        ...
        fortran = True
        scale_rho = True        # Set to true if the number concentration and reflectivity moments need
        ...
        consts['n0rain'] = 8.0e6  # Constant intercept parameter for rain -- This isn't actually used
                                  # in the WDM6 scheme since rain is double-moment, but is set here
                                  # for safety/completeness
        consts['n0hail'] = 4.0e4  # Constant intercept parameter for graupel -- used below to compute Ntg
        consts['n0snow'] = 3.0e6  # Constant intercept parameter for graupel -- used below to compute Ntg
        ...
        micro['nts'] = dsd.cal_Nt(micro['rhoa'],micro['qs'],consts['n0snow'],cs,micro['alphas'])

        # Compute Nth
        cg = (N.pi/6.)*micro['rhoh']
        micro['nth'] = dsd.cal_Nt(micro['rhoa'],micro['qh'],consts['n0hail'],cg,micro['alphah'])

        # Compute Ntr
        cr = (N.pi/6.)*micro['rhor']
        micro['ntr'] = dsd.cal_Nt(micro['rhoa'],micro['qr'],consts['n0rain'],cr,micro['alphar'])
    ...
    def _readwrfmicroWSM6(self):
        ...
        consts['n0rain'] = 8.0e6  # Constant intercept parameter for rain -- This isn't actually used
                                  ...
                                  # for safety/completeness
        consts['n0grpl'] = 4.0e6  # Constant intercept parameter for graupel -- used below to compute Ntg
        ...
        micro['ntg'] = dsd.cal_Nt(micro['rhoa'],micro['qg'],consts['n0grpl'],cg,micro['alphag'])
...
#       return graupel_on,hail_on,rhoa,qr,qs,qg,Ntr,Nts,Ntg,alphar,alphas,alphag,n0rain,n0snow,n0grpl,  \
#       rhos,rhog,tair
        return micro, consts
    ...
    def _readwrfmicroThomp(self):
        ...
        micro['rhos'] = consts['rhoscst'] * N.ones_like(micro['qs']) # Note that rhos is not constant in Thompson scheme but
        ...
        micro['alphar'] = N.zeros_like(micro['qr']) # N.ones_like(qr), looks like Thompson has 0 for alphar by default
        micro['alphas'] = N.zeros_like(micro['qs'])
        micro['alphag'] = N.zeros_like(micro['qg'])
...
#
        ...
        xslw1 = N.where((micro['tair'] < 270.65) & qrmask & (mvd_r > 100.e-6),4.01 + N.log10(mvd_r),0.01)

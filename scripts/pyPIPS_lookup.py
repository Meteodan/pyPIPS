# Creates CSV files for the radar retrievals. Would need to be updated anytime DSDretrieval_radar
# is changed,
...
dbz = np.arange(0.0, 70.0, 0.1)
zdr = np.arange(0.0, 6.0, 0.01)
...
for i in range(np.size(dbz)):
    ...
    W2 = []
    SIGM2 = []
    for j in range(np.size(zdr)):
        (R_retr, D0_retr, mu_retr, lam_retr, N0_retr, Nt_retr, W_retr, sigm_retr, Dm_retr,
         N_retr) = DR.retrieve_DSD(dbz[i], zdr[j], d, fa2, fb2, intv)
        R2.append(R_retr)
        D0_2.append(D0_retr)
        MU2.append(mu_retr)

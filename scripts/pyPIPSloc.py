# Plot_Disdrometer
#
# This script plots several quantities based on disdrometer data from the
...
# TODO: Do I need the below for anything?
...
for index, dis_name, dis_filename, dloc in \
    ...
    print("Lat/Lon/alt of " + dis_name + ": " + "{:7.4f}".format(dloc[0]) +
          " {:7.4f}".format(dloc[1]) + " {:3.0f}".format(dloc[2]))

    # Return time range of disdrometer deployment

    first, last = dis.readPIPStimerange(filepath)
    # print first[0],first[1],last[0],last[1]
    print("Start/End time of deployment for " + dis_name + ": " + first[0] + " " + first[1] +
          " to " + last[0] + " " + last[1])

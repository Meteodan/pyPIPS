...
for index, parsivel_combined_file in enumerate(parsivel_combined_filelist):
    ...
    if not args.calc_for_SATP:
        ...
        ND = parsivel_combined_ds['SATP_ND_{}{}'.format(dataset_name, ND_tag)]
    ...
    fits_ds = xr.Dataset({'{}{}'.format(name, ND_tag): da for name, da in zip(names, data_arrays)})
    ...
    fits_ds = pipsio.combine_parsivel_data(fits_ds, Dm, name='Dm43{}'.format(ND_tag),
                                           coord=coord_to_combine)
    fits_ds = pipsio.combine_parsivel_data(fits_ds, sigma, name='sigma{}'.format(ND_tag),
                                           coord=coord_to_combine)

# plot_parsivel_diag_timeseries.py
#
# This script plots timeseries of diagnostic quantities from the Parsivel telegram
import os
import argparse
...
parser = argparse.ArgumentParser(description="Plots Parsivel diagnostic timeseries from PIPS data")
...
# Create the directory for the diagnostic plots if it doesn't exist
if args.plot_dir:
    plot_dir = args.plot_dir
parsivel_diag_timeseries_image_dir = os.path.join(plot_dir, 'parsivel_diag_timeseries')
if not os.path.exists(parsivel_diag_timeseries_image_dir):
    os.makedirs(parsivel_diag_timeseries_image_dir)
...
for index, parsivel_combined_file in enumerate(parsivel_combined_filelist):
    ...
    image_dir = os.path.join(parsivel_diag_timeseries_image_dir, deployment_name)

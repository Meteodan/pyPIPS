...
import pandas as pd
from modules.utils import log, warning
...
if(pc.comp_radar):
    if(not pc.loadradopt):
        ...
        radar_filelist, radtimes = radar.getradarfilelist(radar_dir, starttime=starttimerad,
                                                          stoptime=stoptimerad, platform=platform,
                                                          el_req=el_req)
        ...
        for index, path, sweeptime in zip(range(len(radar_filelist)), radar_filelist, radtimes):
            ...
            if(platform == 'NEXRAD'):
                (outfieldnames, fields, range_start, radar_range, azimuth_start_rad, azimuth_rad,
                 rlat_rad, rlon_rad, ralt, el_rad) = radar.readCFRadial(True, el_req, rlat, rlon,
                                                                        ralt, path, sweeptime,
                                                                        fieldnames)
            elif(platform == 'SMARTR'):
                (outfieldnames, fields, range_start, radar_range, azimuth_start_rad, azimuth_rad,
                 rlat_rad, rlon_rad, ralt, el_rad) = radar.readCFRadial(False, el_req, rlat, rlon,
                                                                        ralt, path, sweeptime,
                                                                        fieldnames)
            elif(platform == 'UMXP'):
                (outfieldnames, fields, range_start, radar_range, azimuth_start_rad, azimuth_rad,
                 rlat_rad, rlon_rad, ralt, el_rad) = radar.readUMXPnc(path, sweeptime, fieldnames,
                                                                      heading=heading)
            ...
            dxy_list, fields_D = dis.rad2DD2(fields, range_start, radar_range,
                                             azimuth_start_rad, azimuth_rad, rlat_rad, rlon_rad,
                                             ralt, el_rad, dlocs, average_gates=False,
                                             Cressman=True, roi=1000.)
        ...
        if(pc.saveradopt):
            raddate_file = open('radar_files/' + radar_name + '_' +
                                starttimerad.strftime(fmt3).strip() + '_' +
            ...
            radnpz_filename = ('radar_files/' + radar_name + '_' +
                               starttimerad.strftime(fmt3).strip() + '_' +
    ...
    if(pc.plot_radar):
        ...
        for index, path, sweeptime in zip(range(len(radar_filelist)), radar_filelist, radtimes):
            ...
            # array.tolist() yields a nested list for a
            # 2D+ array!
            ...
            # In most of our cases the radar location isn't going to change with time, but in the
            # more general case, this may not be true (i.e. if we are dealing with a mobile radar).
            # print
            # "sweeptime,rlat_rad,rlon_rad,ralt,el_rad",sweeptime.strftime(fmt),rlat_rad,rlon_rad,
            # ralt,el_rad
            print("Radar sweep time: ", sweeptime.strftime(fmt))
            # Prepare masks for fields by reflectivity < some threshold

            for field, fieldname in zip(
                    fields_list, outfieldnames):   # Probably should do this using a dictionary
                if(fieldname == 'dBZ'):
                    mask = N.where(field > 5.0, False, True)

            masklist = [mask, mask, mask]

            # Compute height and range of radar beam above disdrometer (assumes
            # lambert conformal like plotsweep for now)
            for dloc, dname, dxy in zip(dlocs, dis_name_list, dxy_list):
                Dx, Dy = dxy
                print(Dx, Dy)
                h, r = oban.computeheightrangesingle(Dx, Dy, el_req * deg2rad)
                # In most of our cases the radar location isn't going to change with time, but in
                # the more general case, this may not be true (i.e. if we are dealing with a mobile
                # radar).
                print("Disdrometer name,x,y,radar elevation angle,slant range, approximate beam"
                      "height:")
                print(dname, Dx, Dy, el_rad / deg2rad, r, h)
                if(dloc == dlocs[0] and plotxmin == -1):
                    plotlims = [Dx - 25000., Dx + 25000., Dy - 30000., Dy + 20000.]

            figlist, gridlist = radar.plotsweep(radlims, plotlims, outfieldnames, fields_list,
                                                masklist, range_start, radar_range,
                                                azimuth_start_rad, azimuth_rad, rlat_rad, rlon_rad,
                                                ralt, el_rad, False, pc.plot_radar, dis_name_list,
...
for index, dis_filename, dis_name, starttime, stoptime, centertime, dloc in \
        zip(range(0, len(dis_list)), dis_list, dis_name_list, starttimes, stoptimes, centertimes,
            dlocs):
    ...
    filepath = dis_dir + dis_filename
    (datetimesUTC, pdatetimesUTC, flaggedtimes, hailflag, intensities, preciptots, reflectivities,
     pcounts, pcounts2, sensortemps, concentrations, onedrop_concentrations, countsMatrix,
     amplitudes, windspds, winddirrels, winddirabss, winddiags, fasttemps, slowtemps, dewpoints,
     RHs_derived, RHs, pressures, compass_dirs, GPS_lats, GPS_lons, GPS_stats, GPS_alts, voltages,
     DSD_interval, DSD_intervalstr, DSD_index) = \
        dis.readPIPS(filepath, basicqc=pc.basicQC, rainfallqc=pc.rainfallQC,
    ...
    datetimesUTCnums = dates.date2num(datetimesUTC)
    pdatetimesUTCnums = dates.date2num(pdatetimesUTC)
    ...
    if(N.int(starttime) == -1):
        ...
        starttime = dates.date2num(datetime(N.int(starttime[:4]), N.int(starttime[4:6]),
                                   N.int(starttime[6:8]), N.int(starttime[8:10]),
                                   N.int(starttime[10:12])))
    ...
    if(N.int(stoptime) == -1):
        ...
        stoptime = dates.date2num(datetime(N.int(stoptime[:4]), N.int(stoptime[4:6]),
                                  N.int(stoptime[6:8]), N.int(stoptime[8:10]),
                                  N.int(stoptime[10:12])))
        pstoptime = stoptime
        plotstoptime = stoptime
        try:
    ...
    disdates_start.append(disdates_start[-1] + DSD_interval_td)
    ...
    if(pc.timetospace and pc.comp_radar):
        ...
        if(pc.comp_radar):
            plotx_rad = dates.date2num(radtimes)
        plotx_dis = dates.date2num(disdates)
        plotx_dis_start = dates.date2num(disdates_start)
        plotx_dis_avg = dates.date2num(disdates_avg)

    sec_offset = datetimesUTC[0].second
    rho_tDSD = pd.Series(
        data=rho,
        index=datetimesUTC).resample(
        DSD_intervalstr,
        label='right',
        closed='right',
        base=sec_offset).mean().loc[DSD_index].values

    if(pc.calc_DSD):
        # Now for the fun part.  Calculate exponential and gamma distribution size distribution
        # parameters using the method of moments, after Zhang et al. 2008 and Tokay and Short 1996

        synthbins, exp_DSD, gam_DSD, dis_DSD = dis.calc_DSD(pc, min_diameter, avg_diameter,
                                                            max_diameter, bin_width, Nc_bin,
                                                            logNc_bin, rho_tDSD, pc.qrQC,
                                                            pc.qr_thresh, pcounts2)

        # Unpack needed values from returned tuples

        (N_expDSD, N0_exp, lamda_exp, mu_exp, qr_exp, Ntr_exp, refl_DSD_exp, D_med_exp,
         D_m_exp) = exp_DSD
        (N_gamDSD, N0_gam, lamda_gam, mu_gam, qr_gam, Ntr_gam, refl_DSD_gam, D_med_gam, D_m_gam,
         LWC_gam, rainrate) = gam_DSD
        (Nc_bin, logNc_bin, D_med_disd, D_m_disd, D_mv_disd, D_ref_disd, QR_disd, refl_disd,
         LWC_disd, M0) = dis_DSD
        N_gamDSD = N_gamDSD.T
...
# dualpol_dis = dis.calpolrain(wavelength,scattfile,Nc_bin,bin_width) #
# use for raw distribution
            dualpol_dis = dis.calpolrain(
                wavelength,
                scattfile,
                (N_gamDSD / 1000.),
                bin_width)  # use for gamma fit
    ...
    if(pc.plot_DSD_meteo):
        ...
        DSDstarttimes = dates.date2num(disdates_start[pstartindex:pstopindex + 1])
        DSDmidtimes = dates.date2num(disdates_avg[pstartindex:pstopindex + 1])
...
#        logNc_bin_plot = logNc_bin[:,pstartindex:pstopindex+1] # use for raw distribution
        ...
        # and otherwise ignores data in all bins larger than 9 mm.  For this reason it is
        # recommended to first set the "rain_only_QC" flag to True in disdrometer_module.py

        if(pc.calc_DSD):
            # 3-min average of median volume diameter and radar reflectivity (Rayleigh
            # approx.) from disdrometer

            window = int(180. / DSD_interval)
# D_0_dis_avg =
# pd.Series(D_med_disd).rolling(window=window,center=True,win_type='triang',min_periods=1).mean().values
# # use for raw distribution
            D_0_dis_avg = pd.Series(D_med_gam).rolling(
                window=window,
                center=True,
                win_type='triang',
                min_periods=1).mean().values  # use for gamma fit
# dBZ_ray_dis_avg =
# pd.Series(refl_disd).rolling(window=window,center=True,win_type='triang',min_periods=1).mean().values
# # use for raw distribution
            # use for gamma fit
            dBZ_ray_dis_avg = pd.Series(refl_DSD_gam).rolling(
                window=window, center=True, win_type='triang', min_periods=1).mean().values
        ...
        if(pc.plot_diagnostics):
            ...
            flaggedtimes_plot = DSDmidtimes[flaggedtimes_index]
...
if (pc.plot_retr_PPI):
    ...
    for index, path, sweeptime in zip(range(len(radar_filelist)), radar_filelist, radtimes):
        ...
        # print
        # "sweeptime,rlat_rad,rlon_rad,ralt,el_rad",sweeptime.strftime(fmt),rlat_rad,rlon_rad,ralt,
        # el_rad
        print("Radar sweep time: ", sweeptime.strftime(fmt))
        # Prepare masks for fields by reflectivity < some threshold
        # Probably should do this using a dictionary
        for field, fieldname, field_D in zip(
                fields_list, outfieldnames, fields_D_list):
            if(fieldname == 'dBZ'):
                mask = N.where(field > 5.0, False, True)
                rad_dBZ = field
                dis_dBZ = field_D
        ...
        for n2 in range(0, ng):
            (r_retr, d0_retr, mu_retr, lam_retr, n0_retr, nt_retr, w_retr, sigm_retr, dm_retr,
             n_retr) = DR.retrieve_DSD(dis_dBZ[n2], dis_ZDR[n2], d, fa2, fb2, intv, wavelength)
            r_dis.extend(r_retr)
        ...
        figlist, gridlist = radar.plotsweep(radlims, plotlims, outfieldnames_retr,
                                            fields_list_retr, masklist,
                                            range_start, radar_range, azimuth_start_rad,
                                            azimuth_rad, rlat_rad, rlon_rad, ralt, el_rad, False,
...
#         # In most of our cases the radar location isn't going to change with time, but in the more
#         # general case, this may not be true (i.e. if we are dealing with a mobile radar).
#         #print "sweeptime,rlat_rad,rlon_rad,ralt,el_rad",sweeptime.strftime(fmt),rlat_rad,
#                 rlon_rad,ralt,el_rad
#         print "Radar sweep time: ",sweeptime.strftime(fmt)
#         # Prepare masks for fields by reflectivity < some threshold
#         # Probably should do this using a dictionary
#         for field,fieldname,field_D in zip(fields_list,outfieldnames,fields_D_list):
#             if(fieldname == 'dBZ'):
#                 mask = N.where(field > 5.0,False,True)
#                 rad_dBZ = field
#                 dis_dBZ = field_D
...
#             (r_retr,d0_retr,mu_retr,lam_retr,N0_retr,Nt_retr,W_retr,sigm_retr,Dm_retr,N_retr) = \
#                   DR.retrieve_DSD(rad_dBZ[n1,:],rad_ZDR[n1,:],d,fa2,fb2,intv)
#             R_retr[n1] = r_retr
#             D0_retr[n1] = d0_retr
...
#         print D0_retr.shape
#
#         # Compute height and range of radar beam above disdrometer (assumes lambert conformal
#         # like plotsweep for now)
#         for dloc,dname,dxy in zip(dlocs,dis_name_list,dxy_list):
#             Dx,Dy = dxy
#             print Dx,Dy
#             h,r = oban.computeheightrangesingle(Dx,Dy,el_req*deg2rad)
#             # In most of our cases the radar location isn't going to change with time, but in the
#             # more general case, this may not be true (i.e. if we are dealing with a mobile
#             # radar).
#             #print "Disdrometer name,x,y,radar elevation angle,slant range, approximate beam
#                     height:"
#             #print dname,Dx,Dy,el_rad/deg2rad,r,h
...
#                 plotlims = [Dx-25000.,Dx+25000.,Dy-30000.,Dy+20000.]
#
# #         R_dis_retr,D0_dis_retr,mu_dis_retr,lam_dis_retr,N0_dis_retr,Nt_dis_retr,W_dis_retr,
#           sigm_dis_retr,Dm_dis_retr,N_dis_retr = DR.retrieve_DSD(dBZ,ZDR,d,fa2,fb2,intv)
# #         fields_D_list_retr = [R_dis_retr,D0_dis_retr]
# #         # ^^^ this is incorrect and does nothing relevant right now
#
#         r_retr,d0_retr,mu_retr,lam_retr,N0_retr,Nt_retr,W_retr,sigm_retr,Dm_retr,N_retr = \
#           DR.retrieve_DSD(dis_dBZ,dis_ZDR,d,fa2,fb2,intv)
#         fields_D_list_retr = [r_retr,d0_retr]
#
#         figlist,gridlist = radar.plotsweep(radlims,plotlims,outfieldnames_retr,fields_list_retr,
#                                            masklist, range_start,radar_range,azimuth_start_rad,
#                                            azimuth_rad,rlat_rad,rlon_rad,ralt,el_rad,False,
#                                            pc.plot_radar,dis_name_list,dxy_list,
#                                            fields_D_list_retr)
#
#             # Save figures
#         for fieldname,fig in zip(outfieldnames_retr,figlist):
#             plt.figure(fig.number)
#             plt.title(sweeptime.strftime(fmt2).strip())
#             plt.savefig(image_dir+'radar_PPI/'+fieldname+sweeptime.strftime(fmt3).strip()+
#                         'el'+str(el_req)+'.png',dpi=200,bbox_inches='tight')
#             plt.close(fig)

...
avg_diameter = 0.5*(min_diameter + max_diameter)
min_fall_bins = pp.parsivel_parameters['min_fallspeed_bins_mps']
max_fall_bins = pp.parsivel_parameters['max_fallspeed_bins_mps']
fall_bins = 0.5*(min_fall_bins + max_fall_bins)
...
for index, dis_filename, dis_name, starttime, stoptime, centertime, dloc, ptype, conv_df, \
    ...
    # logND = np.ma.masked_where(ND < ND_onedrop, logND)
...
#             xloc_wind_1min = [(x*pc.stormmotion)/1000.0 for x in delta_t_wind_1min]
        ...
        plotx_rad = np.array(xloc_rad)
        plotx_dis = np.array(xloc_dis)
        plotx_dis_start = np.array(xloc_dis_start)
        plotx_dis_avg = np.array(xloc_dis_avg)
#         if(plot_thermo):
#             plotx_thermo = np.array(xloc_thermo)
#             plotx_thermo_1min = np.array(xloc_thermo_1min)
#             plotx_wind = np.array(xloc_wind)
#             plotx_wind_1min = np.array(xloc_wind_1min)
    else:
        if(pc.comp_radar):
...
#             plotx_wind_1min = dates.date2num(winddates_1min)
    ...
    # np.savetxt('ND1minavg.txt', ND1minavg) # Commented out for now
    ...
    if(pc.calc_DSD):
        ...
        logND_expDSD = np.ma.masked_where(ND_expDSD < ND_onedrop, logND_expDSD)

        ND_gamDSD = ND_gamDSD.T
        logND_gamDSD = np.ma.log10(ND_gamDSD / 1000.)  # Get to log(m^-3 mm^-1)
        logND_gamDSD = np.ma.masked_where(ND_gamDSD < ND_onedrop, logND_gamDSD)
        ...
        for DSDtype in ['observed', 'exponential', 'gamma']:
            ...
            if(pc.plot_diagnostics and ptype not in 'NV2'):
                if(DSDtype == 'observed'):
                    ...
                    hailflag_index = np.where(PSD_plot_df['hailflag'].values)[0]
            ...
            if(pc.plot_only_precip and pc.calc_dualpol):
                ...
                rainindex = np.where(disvars['RHV'] > 0.1)
    ...
    if(pc.plot_DSDs):
        ...
        for t in range(np.size(ND, axis=1)):

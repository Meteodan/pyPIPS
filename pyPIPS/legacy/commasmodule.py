...
class COMMASDataHandler(DataHandler):
    ...
    def loadGrid(self):
        ...
        xc = N.lib.stride_tricks.as_strided(xc, strides=((0, 0) + xc.strides),
                                            shape=((nzm, nym) + xc.shape))
        xe = N.lib.stride_tricks.as_strided(xe, strides=((0, 0) + xe.strides),
                                            shape=((nzm, nym) + xe.shape))

        yc = N.lib.stride_tricks.as_strided(yc, strides=((0, ) + yc.strides + (0, )),
                                            shape=((nzm, ) + yc.shape + (nxm, )))
        ye = N.lib.stride_tricks.as_strided(ye, strides=((0, ) + ye.strides + (0, )),
                                            shape=((nzm, ) + ye.shape + (nxm, )))

        zc = N.lib.stride_tricks.as_strided(zc1d, strides=(zc1d.strides + (0, 0)),
                                            shape=(zc1d.shape + (nym, nxm)))
        ze = N.lib.stride_tricks.as_strided(ze1d, strides=(ze1d.strides + (0, 0)),
                                            shape=(ze1d.shape + (nym, nxm)))
    ...
    def loadTimes(self):
        ...
        datetimestart = datetime.datetime.strptime(timestringstart,
        ...
        for i in range(1, len(self._filetimes)):
    ...
    def loadMicrophysics(self):
        ...
        if microphys == "LFO":
            ...
            if(microphys == "ZVDM" or microphys == "ZVDHM" or microphys == "ZIEGM" or
               microphys == "ZIEGHM"):
                consts['MFflg'] = 2
            if(microphys == "ZVDM" or microphys == "ZVD" or microphys == "ZIEG"):
            ...
            micro['alphar'] = N.zeros_like(qr)
            micro['alphas'] = N.zeros_like(qr)
            micro['alphag'] = N.zeros_like(qr)
            micro['alphah'] = N.zeros_like(qr)
            micro['qsw'] = N.zeros_like(qr)
            micro['qgw'] = N.zeros_like(qr)
            micro['qhw'] = N.zeros_like(qr)
            micro['rhos'] = rhoscst
            micro['rhog'] = rhogcst
            micro['rhoh'] = rhohcst
    ...
    def _readcommasmicroMY(self):
        ...
        Also checks if the sixth moments are present and computes the shape parameter as
        appropriate.
        ...
        for k in range(0, zc.size):
            exner[k] = self._file.variables['PI'][self._timeindex, k, :, :] + \
                self._file.variables['PIINIT'][self._timeindex, k]
        ...
        for spec, name, ncname in [('r', 'rain', 'RW'), ('s', 'snow', 'SW'),
                                   ('g', 'grpl', 'HW'), ('h', 'hail', 'HL')]:
            ...
            micro['rho' + spec] = self._file.variables['RHO_Q%s_MY' % spec.upper()][:] * \
                N.ones_like(micro['q' + spec])
            ...
            try:
                ...
                mu = 1. / 3.
                micro['alpha' + spec] = dualpol.solve_alpha_iter(micro['rhoa'], mu,
                                                                 micro['q' + spec],
                                                                 micro['nt' + spec],
                                                                 micro['z' + spec],
                                                                 micro['rho' + spec])
                ...
                print("Did not find Z%s array! Assuming 2-moment with constant shape parameter." %
                      spec.upper())
                micro['zr'] = N.zeros_like(micro['q' + spec])
                try:
                    micro['alpha' + spec] = self._file.variables['ALPHA%s_MY' % spec.upper()][:] * \
                                          N.ones_like(micro['q' + spec])
                except BaseException:
                    micro['alpha' + spec] = N.zeros_like(micro['q' + spec])
    ...
    def _readcommasmicroLFO(self):
        ...
        p = self._file.variables['P'][self._timeindex, :, :, :]
        pt = self._file.variables['TH'][self._timeindex, :, :, :]
        qv = self._file.variables['QV'][self._timeindex, :, :, :]
        zc = self._file.variables['ZC'][...]
        ...
        for spec, name, ncname in [('r', 'rain', 'R'), ('s', 'snow', 'S'), ('g', 'grpl', 'H')]:
            ...
            # Read in particle densities
            micro['rho' + spec] = self._file.variables['RHO_Q' +
                                                       ncname][:] * N.ones_like(micro['q' + spec])
    ...
    def _readcommasmicroZVD(self):
        """
        Reads in several microphysical variables for the ZVD scheme and derives others from a COMMAS
        netCDF file. Determines if graupel and hail are present and returns the appropriate flags.
        Also checks whether liquid water fraction, variable density, and sixth moments are present
        ...
        for k in range(0, zc.size):
            exner[k] = self._file.variables['PI'][self._timeindex, k, :, :].astype(
                N.float64) + self._file.variables['PIINIT'][self._timeindex, k]
        ...
        for spec, name, ncname in [('r', 'rain', 'RW'), ('s', 'snow', 'SW'),
            ...
            try:
                consts['n0' + name] = self._file.variables['CNO' +
                                                           ncname.strip('W')][:].astype(N.float64)
            except BaseException:
                consts['n0' + name] = 0.0

            # Read in particle densities
            try:
                micro["rho%scst" % spec] = self._file.variables['RHO_Q' + spec.upper()][:] * \
                    N.ones_like(micro['q' + spec])
            except BaseException:
            ...
            if spec != 'r':
                ...
                try:
                    micro["q%sw" %
                          spec] = self._file.variables["Q%sW" %
                                                       ncname.strip('W')][self._timeindex, :, :, :]
                except BaseException:
                    micro["q%sw" % spec] = N.zeros_like(micro['q' + spec])
            ...
            try:
                ...
                if spec == 'r' and consts['imurain'] == 3:
                    ...
                    if spec == 'r':
                        print("Rain is gamma-diameter.")
                    mu = 1. / 3.

                micro['alpha' +
                      spec] = dualpol.solve_alpha_iter(micro['rhoa'], mu, micro['q' + spec],
                                                       micro['nt' + spec], micro['z' + spec],
                                                       micro['rho' + spec])
                print("Found Z%s array! Computed shape parameter for %s." % (spec.upper(), name))
            except BaseException:
                print("Did not find Z%s array! Assuming 2-moment with constant shape parameter." %
                ...
                try:
                    micro['alpha' + spec] = self._file.variables['ALPHA' + ncname.strip('W')][:] * \
                                            N.ones_like(micro['qr'])
                except BaseException:
                    if spec in ['r', 's']:
    ...
    def _readcommasmicroTAK(self):
        """
        Reads in several microphysical variables for the TAK scheme and derives others from a COMMAS
        netCDF file
        """
        ...
        # print rs_bin
        # hs_bin = 1.0e-4 * N.exp(N.tile(N.arange(0., 5., 1.), (21, 1)).transpose() /
        #                         (4. / N.log(2. * rs_bin / 2.0e-3)))    # Snow/Ice thickness
        # print hs_bin
        ...
        for i in range(34):
            if(i == 0):
                ...
                varname = 'CRW%02d' % (i + 1)
            # Convert from cm^-3 to m^-3/m
            nrtmp = self._file.variables[varname][self._timeindex, :, :, :] * 1.e6 / rbinwidth[i]
        ...
        for k in range(5):
            for i in range(21):
                ...
                nstmp = self._file.variables[varname][self._timeindex,
                                                      :, :, :] * 1.e6 / sbinwidth[k, i]
        ...
        for i in range(45):
            ...
            ngtmp = self._file.variables[varname][self._timeindex, :, :, :] * 1.e6 / gbinwidth[i]
        ...
        for i in range(45):
            if(i == 0):
                ...
                varname = 'CHL%02d' % (i + 1)
            # Convert from cm^-3 to m^-3
            nhtmp = self._file.variables[varname][self._timeindex, :, :, :] * 1.e6 / hbinwidth[i]
    ...
    def commascalradTmat(self, microphysics, MFflg, timeindex, filename, wavelen, dirscatt,
                         fortran=False):
        ...
        if (microphysics[:2] == "MY"):
            ...
            micro, consts = self._readcommasmicroMY(timeindex, filename, fortran)
        elif (microphysics[:3] == "ZVD"):
            MPflg = 1
            print("Reading microphysics information for ZVD scheme")
            micro, consts = self._readcommasmicroZVD(timeindex, filename, fortran)
        else:
            print("Microphysics not supported yet!")
            sys.exit()

        dualpol.dualpara.setgrplhl(consts['graupel_on'], consts['hail_on'])

        print("Initializing DSD parameters")
        # dualpol.dualpara.init_dsd()
        dualpol.dualpara.model_dsd(consts['n0rain'], consts['n0snow'], consts['n0grpl'],
                                   consts['n0hail'], consts['rhosnow'], consts['rhogrpl'],
                                   consts['rhohail'])

        print("Calling dualpol subroutine")
        logz, sumzh, sumzv, logzdr, sumzhv, kdp, ahh, avv = \
            dualpol.refl_rsa_array(MPflg, MFflg, dirscatt, wavelen, micro['rhoa'], micro['qr'],
                                   micro['qs'], micro['qg'], micro['qh'], micro['Ntr'],
                                   micro['Nts'], micro['Ntg'], micro['Nth'], micro['alphar'],
                                   micro['alphas'], micro['alphag'], micro['alphah'], micro['qsw'],
                                   micro['qgw'], micro['qhw'], micro['rhog'], micro['rhoh'])

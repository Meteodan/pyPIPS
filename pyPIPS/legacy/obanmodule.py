...
def intrp2swp(swp_height, model_z, var):
    ...
    swpvar = np.zeros_like(var[..., 0])

    # vec_vintrp = np.vectorize(vintrp)
    ...
    for index, height in np.ndenumerate(swp_height):
        k = np.searchsorted(model_z, height)
...
def findswpindices(swp_height, model_z):
    ...
    z_index = np.zeros_like(swp_height, dtype='int32')
    ...
    for index, height in np.ndenumerate(swp_height):
        ...
        z_index[index] = max(np.searchsorted(z_column, height) - 1, 0)
...
def intrp2swp2(swp_height, model_z, var):
    ...
    for index, height in np.ndenumerate(swp_height):
        ...
        swpvar[index] = np.interp(swp_height[index], z_column, var[index[0], index[1], :])
...
def intrp2swp3(swp_height, model_z, var):
    ...
    swp_var = np.zeros_like(swp_height)

    for index, dummy in np.ndenumerate(swp_height[..., 0]):
...
def computeheightrangesingle(x, y, el):
    ...
    height and slant path range of the radar beam at a single x,y location.  Modified from OPAWS
    compute_height function
    """
    r = np.sqrt(x * x + y * y) / np.cos(el)  # Slant range
    h = np.sqrt(r * r + erem * erem + 2.0 * r * erem * np.sin(el)) - erem
    return h, r
...
def computeheight2(x, y, elev_list):
    ...
    swp_height = np.zeros((np.size(X, 0), np.size(X, 1), len(elev_list)))

    for k, el in enumerate(elev_list):
        r = np.sqrt(X * X + Y * Y) / np.cos(el)   # Slant range
        swp_height[..., k] = np.sqrt(r * r + erem * erem + 2.0 * r * erem * np.sin(el)) - erem
...
def llobs(r, az, el, rlat, rlon, ralt):
    ...
    h = np.sqrt(r * r + erem * erem + 2.0 * r * erem * np.sin(el)) - erem
    h = h + ralt
    s = erem * np.arcsin((r * np.cos(el)) / (erem + h))
    d = s / rem

    # Now compute the observation lat/lon
    lat = np.arcsin(np.sin(rlat) * np.cos(d) + np.cos(rlat) * np.sin(d) * np.cos(az))
    dlon = np.arctan2(np.sin(az) * np.sin(d) * np.cos(rlat), np.cos(d) - np.sin(rlat) * np.sin(lat))
    lon = np.mod(rlon + dlon + np.pi, 2.0 * np.pi) - np.pi
...
def ll_to_xy(lat, lon, rlat, rlon, map_proj=0):
    ...
    if(map_proj == 0):                              # Flat Earth approximation
        x = rem * np.cos(0.5 * (rlat + lat)) * (lon - rlon)
        y = rem * (lat - rlat)
    elif(map_proj == 1):                            # Lambert conformal
        tlat = deg2rad * tlat1
        blat = deg2rad * tlat2
        cenlon = clon
        b = (np.log(np.sin(tlat)) - np.log(np.sin(blat))) / \
            (np.log(np.tan(tlat / 2.)) - np.log(np.tan(blat / 2.)))
        c1 = rem * np.sin(tlat) / (b * np.tan(tlat / 2.)**b)
        # first find the x and y of the first point
        xlat = (np.pi / 2.) - rlat
        xlon = rlon
        d = c1 * (np.sin(xlat) / (1. + np.cos(xlat)))**b
        x1 = d * np.sin(b * (xlon - cenlon))
        y1 = d * np.cos(b * (xlon - cenlon))
        # now find the x and y of the second point
        xlat = (np.pi / 2.) - lat
        xlon = lon
        d = c1 * (np.sin(xlat) / (1. + np.cos(xlat)))**b
        x2 = d * np.sin(b * (xlon - cenlon))
        y2 = d * np.cos(b * (xlon - cenlon))
...
def readsweep(obanfilename, stime, target_elev, timetol):
    ...
    requested time.  Returns dBZ, Zdr, Kdp, and Vr if they exist, otherwise just return False
    booleans
    """
    ...
    print(
        "Difference in seconds of requested model time and oban start time = ",
        model_time_sec_from_oban_time)
    ...
    # Now, search through the file and find the sweeps closest to the target sweep for the given
    # variable. Store the data and times of each sweep in lists, which will be used later to attempt
    # to match times from the given model netCDF file as well as the target time
    ...
    for p in [var_shape[1] - 1]:
        ...
        # for k in np.arange(0, var_shape[2], options.skip):
        for k in np.arange(0, var_shape[2]):
            elev_mask = np.ma.masked_array(
                elev_array[0, k, :, :], elev_array[0, k, :, :] <= -32767.)
            mean_elev = elev_mask.mean()
            time_mask = np.ma.masked_array(
                time_array[0, k, :, :], time_array[0, k, :, :] <= -32767.)
            ...
            if(abs(mean_elev - target_elev) <= 0.1):
                ...
                # Look for a sweep time that is within the time tolerance of the requested model
                # time and corresponds to the target elevation
                if(model_time_diff <= timetol and model_time_diff < model_time_diff_min):
                    ...
                    # height_swp = np.ma.masked_array(height_array[0,k,...],
                    #                                 height_array[0,k,...] <= -32767.)
                    height_swp = height_array[0, k, ...]
                    if(dBZ_found):
                        dBZ_swp = np.ma.masked_array(
                            dBZ[0, p, k, ...], dBZ[0, p, k, ...] <= -32767.)
                    if(Zdr_found):
                        Zdr_swp = np.ma.masked_array(
                            Zdr[0, p, k, ...], Zdr[0, p, k, ...] <= -32767.)
                    if(Kdp_found):
                        Kdp_swp = np.ma.masked_array(
                            Kdp[0, p, k, ...], Kdp[0, p, k, ...] <= -32767.)
                    if(rhv_found):
                        rhv_swp = np.ma.masked_array(
                            rhv[0, p, k, ...], rhv[0, p, k, ...] <= -32767.)
                    if(Vr_found):
                        Vr_swp = np.ma.masked_array(Vr[0, p, k, ...], Vr[0, p, k, ...] <= -32767.)

                    print("Slice: %3d  Sweep elevation:  %4.2f  Sweep time %s " %
                          (k, mean_elev, sec_utime.num2date(mean_time)))
                    print("Requested model time %s " %
                          sec_utime.num2date(model_time_sec_from_oban_time))
                ...
                # if mean_time != False and np.size(np.where(obanvar2.mask != False)) > 0:
                # files.append(plotoban(dd, vv, x, y, el.mean(), az, sec_utime.num2date(ti.mean()),
                #              pass_no = p+1, directory = options.dir))
...
#        for n, value in np.ndenumerate(dis[indices]):
        for i, j in zip(indices[0], indices[1]):
            n = (i, j)
            value = dis[n]
            if(np.isfinite(obs[n])):
                # print value
                # print n
                rk2 = value**2.0
                wk = (R2 - rk2) / (R2 + rk2)

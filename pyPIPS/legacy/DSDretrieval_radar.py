# DSDretrieval for radar
# To be used with pyPIPS_lookup and pyPIPS_radar because this requires the individual stepping
# through of Zh and ZDR
...
# terminal velocity equation from Brandes et al. 2002
    v = 3.778 * d**0.67
...
# iteration initialization
    dlm = 0.1
    mt = int(30 / dlm)        # these are the experimental lambda values to run through
    fp = -1.0       # used to test if observed Zdr greater than calculated
    lam1 = 1    # initial value
    zdr = 10.**(Zdr / 10)     # linear value
    refl = 10.**(Z / 10)      # linear value
    # dBZ = Z
    dbzdr = Zdr
    dmx = Dmx
    if np.all([refl >= 1, dmx >= 0.4, zdr >= 1]):     # when dBZ and ZDR are greater than 0
        if (dbzdr < 0.1):   # use empirical relations when ZDR < 0.1 dB from Cao et al. 2008
            ...
            for n3 in range(1, mt):     # run through possible lambda values
                ...
                # mum = -0.008794*lam**2.+1.10139*lam-2.2781      # solve for mu using
                # constrained gamma relation from Cao et al. 2008
            ...
            if(tf == 0):
                ...
                # mum = -0.008794*lam**2.+1.10139*lam-2.2781      # constrained gamma
                # relation again
                mum = -0.0324 * lam**2. + 1.1931 * lam - 1.7023
                zh0 = 0.0
                # tkd = 0.0

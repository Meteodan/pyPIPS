...
class ARPSDataHandler(DataHandler):
    ...
    def setRun(self, file_base, member=0, time=None):
        ...
        file_name = self.fileNameBuilder(os.path.join(
            self._base_dir, file_base), time, member)
        grdbas_name = self.fileNameBuilder(
            os.path.join(self._base_dir, file_base), None, member)
        if self._file_name != file_name:
            if self._file:
                self._file.close()
            if(time == -1):  # Try to open grdbas file instead of time-dependent file
                # Need to add support for netCDF4-python later.
                self._file = Nio.open_file(grdbas_name, format='hdf')
            else:
                # Need to add support for netCDF4-python later.
                self._file = Nio.open_file(file_name, format='hdf')
    ...
    def setTime(self, time):
        ...
        try:
            ...
            # TODO: Since ARPS stores times in separate files, need
    ...
    def loadGrid(self):
        ...
        zcagl = zcagl.T
        zeagl = zeagl.T
        # Quick and dirty approximate heights AGL
        zc1d = N.mean(N.mean(zcagl, axis=1), axis=1)
...
#             ye_rel = ye-y_rad
#
#         # A little trick to duplicate the xs and ys over the vertical coordinate without actually
#           using any more memory.
#         xc_rel = N.lib.stride_tricks.as_strided(xc_rel, strides=((0, ) + xc_rel.strides),
#           shape=((nz-1, ) + xc_rel.shape))
#         yc_rel = N.lib.stride_tricks.as_strided(yc_rel, strides=((0, ) + yc_rel.strides),
#           shape=((nz-1, ) + yc_rel.shape))
#         xe_rel = N.lib.stride_tricks.as_strided(xe_rel, strides=((0, ) + xe_rel.strides),
#           shape=((nz-1, ) + xe_rel.shape))
#         ye_rel = N.lib.stride_tricks.as_strided(ye_rel, strides=((0, ) + ye_rel.strides),
#           shape=((nz-1, ) + ye_rel.shape))

        xc = N.lib.stride_tricks.as_strided(xc, strides=(
            (0, ) + xc.strides), shape=((nz - 1, ) + xc.shape))
        yc = N.lib.stride_tricks.as_strided(yc, strides=(
            (0, ) + yc.strides), shape=((nz - 1, ) + yc.shape))
        xe = N.lib.stride_tricks.as_strided(xe, strides=(
            (0, ) + xe.strides), shape=((nz - 1, ) + xe.shape))
        ye = N.lib.stride_tricks.as_strided(ye, strides=(
            (0, ) + ye.strides), shape=((nz - 1, ) + ye.shape))
        xcor = N.lib.stride_tricks.as_strided(xcor, strides=(
            (0, ) + xcor.strides), shape=((nz - 1, ) + xcor.shape))
        ycor = N.lib.stride_tricks.as_strided(ycor, strides=(
            (0, ) + ycor.strides), shape=((nz - 1, ) + ycor.shape))

        return xc, yc, zc, zc1d, xe, ye, ze, ze1d, bgmap

    def loadTimes(self):
        # First time setup only
        # for ARPS, each time is in a separate file, so we just use
        filetimes = N.array(self._time_list)
        ...
        # Will contain list of times in seconds since model start time in file.
        filetimelist = []
    ...
    def loadMicrophysics(self):
        ...
        if self._microphys[0:2] == 'WR':
            ...
            micro, consts = self.readarpsmicroWR()
    ...
    def _readarpsmicroparam(self):
        ...
        return (n0rain, n0snow, n0grpl, n0hail, rhoice, rhosnow, rhogrpl, rhohail, ntcloud,
                alpharain, alphaice, alphasnow, alphagrpl, alphahail)

    def readarpsmicroWR(self):
        """
        Reads in several microphysical variables for the ARPS Kessler warm-rain scheme and
        derives others from an ARPS HDF file.
        ...
        n0rain, n0snow, n0grpl, n0hail, rhoicst, rhoscst, rhogcst, \
            rhohcst, ntcloud, alpharcst, alphaicst, alphascst, alphagcst, alphahcst = \
            self._readarpsmicroparam()
        if(verbose):
            print(('n0rain = ', n0rain))
            print(('n0snow = ', n0snow))
            print(('n0grpl = ', n0grpl))
        ...
        for var, data in list(micro.items()):
    ...
    def _readarpsmicroMY(self):
        """
        Reads in several microphysical variables for the MY scheme and derives others from an
        ARPS HDF file.  Also checks if the sixth moments are present and computes the shape
        ...
        micro['qh'] = self._hdfreadvar3d('qh', stag='s')
        # TODO: This should be present in the file now, so should update this to read from file...
        consts['graupel_on'] = 1
        # TODO: This should be present in the file now, so should update this to read from file...
        consts['hail_on'] = 1
        ...
        try:
            ...
            mu = 1. / 3.
            micro['alphar'] = dualpol.solve_alpha_iter(
                micro['rhoa'], mu, micro['qr'], micro['ntr'], micro['zr'], micro['rhor'])
            micro['alphas'] = dualpol.solve_alpha_iter(
                micro['rhoa'], mu, micro['qs'], micro['nts'], micro['zs'], micro['rhos'])
            micro['alphag'] = dualpol.solve_alpha_iter(
                micro['rhoa'], mu, micro['qg'], micro['ntg'], micro['zg'], micro['rhog'])
            micro['alphah'] = dualpol.solve_alpha_iter(
                micro['rhoa'], mu, micro['qh'], micro['nth'], micro['zh'], micro['rhoh'])
            ...
            if(self._microphys == 'MY2DA'):
                ...
                dmr = (micro['rhoa'] * micro['qr'] / ((N.pi / 6.) *
                                                      micro['rhor'] * micro['nr']))**(1. / 3.)
                micro['alphar'] = c1r * N.tanh(c2r * (dmr - c3r)) + c4r
                # dmi = (micro['rhoa'] * micro['qi'] / ((N.pi / 6.) *
                #                                       micro['rhoi'] * micro['ni']))**(1. / 3.)
                micro['alphai'] = c1i * N.tanh(c2i * (dmr - c3i)) + c4i
                dms = (micro['rhoa'] * micro['qs'] / ((N.pi / 6.) *
                                                      micro['rhos'] * micro['ns']))**(1. / 3.)
                micro['alphas'] = c1s * N.tanh(c2s * (dms - c3s)) + c4s
                dmg = (micro['rhoa'] * micro['qg'] / ((N.pi / 6.) *
                                                      micro['rhog'] * micro['ng']))**(1. / 3.)
                micro['alphag'] = c1g * N.tanh(c2g * (dmg - c3g)) + c4g
                dmh = (micro['rhoa'] * micro['qh'] / ((N.pi / 6.) *
                                                      micro['rhoh'] * micro['nh']))**(1. / 3.)
                micro['alphah'] = N.where(
                    dmh < 8e-3, c1h * N.tanh(c2h * (dmh - c3h)) + c4h, c5h * dmh - c6h)
            else:
                print(
                    "Did not find Z arrays! Assuming 2-moment with constant shape parameter.")
            micro['zr'] = N.zeros_like(pt)
            micro['zs'] = N.zeros_like(pt)
    ...
    def _readarpsmicroZVD(self):
        """
        Reads in several microphysical variables for the ZVD scheme and derives others from an ARPS
        hdf file.
        Determines if graupel and hail are present and returns the appropriate flags.  Also checks
        if sixth moments are present and computes the shape parameter as appropriate.Note, for now
        qi and qc are ignored since they aren't used in the polarimetric emulator, but obviously
        need to add these for use by other applications.
        """
        ...
        try:
            ...
            micro['rhog'] = micro['qg'] * micro['rhoa'] / micro['vg']
        except BaseException:
            micro['rhog'] = rhogcst * N.ones_like(pt)

        try:
            micro['vh'] = self._hdfreadvar3d('vh', stag='s')
            micro['rhoh'] = micro['qh'] * micro['rhoa'] / micro['vh']
        except BaseException:
            micro['rhoh'] = rhohcst * N.ones_like(pt)
